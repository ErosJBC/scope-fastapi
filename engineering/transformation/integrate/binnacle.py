...
class BinnacleIntegrator:
    ...
    @staticmethod
    def filter_options(dataframe: pd.DataFrame, options: dict[str, Any]) -> pd.DataFrame:
        """
        ...
        filtered_df: pd.DataFrame = dataframe[
            (dataframe['SI/SO'] == options['selected_liquidation']) & (dataframe['DES_ZNJE'] == options['nodo']) &
    ...
    def create_upper_tm_column(dataframe: pd.DataFrame) -> pd.DataFrame:
        ...
        :rtype: pd.DataFrame
        """
        dataframe['UPPER_TM'] = dataframe['CONDICION'].str.split(' - ').str[1].str.split(' ').str[0].str.replace('K',
                                                                                                                 '').astype(
            float) * 1000
        return dataframe
    ...
    def get_list_month_by_period(dataframe: pd.DataFrame, selected_month: str) -> list[str]:
        ...
        list_month_temp = list(range(1, int(selected_month) + 1))[-period_dict[period]:]
    ...
    def create_validators_column(
        ...
        :rtype: pd.DataFrame
        """
        validators: list[str] = ['COD_ZDES', 'ETAPA', 'FAMILIA', 'COD_PRODUCTO']
        validators_df: pd.DataFrame = binnacle.copy()
        for index, column in enumerate(validators):
            expanded = []
            for _, row in binnacle.copy().iterrows():
                if row[column] == 'Todo':
