...
def generate_summary_rebate_sheet(
    ...
    if options["nodo"] == "D. COPACIGULF": cols_to_aggregate['APORTE'] = 'sum'
...
def generate_summary_commercial_recognition_sheet(sellin: pd.DataFrame) -> dict[str, pd.DataFrame]:
    ...
    summary_sheet['MONTH'] = summary_sheet['MONTH'].replace(constants.MONTH)
    summary_sheet.columns = ['CLIENTE', 'MES', 'TMS', 'USD', 'APORTE']
    summary_sheet.loc['TOTAL', 'TMS'] = summary_sheet['TMS'].sum()
    summary_sheet.loc['TOTAL', 'USD'] = summary_sheet['USD'].sum()
    summary_sheet.loc['TOTAL', 'APORTE'] = summary_sheet['APORTE'].sum()
...
def generate_summary_sellout_sheet(sellout: pd.DataFrame, pivot: pd.DataFrame) -> dict[str, pd.DataFrame]:
    summary_sheet: pd.DataFrame
    if sellout['APLICACION'].nunique() == 1:
    ...
    if 'Bonif. P.Base' in list(pivot.columns):
        summary_sheet.loc['TOTAL', f'APORTE Bonif. P.Base'] = summary_sheet[f'APORTE Bonif. P.Base'].sum()
    if 'Bonif. P.Neto' in list(pivot.columns):
        summary_sheet.loc['TOTAL', f'APORTE Bonif. P.Neto'] = summary_sheet[f'APORTE Bonif. P.Neto'].sum()
