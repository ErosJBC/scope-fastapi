...
class SellinIntegrator(ABC):
    ...
    def filter_options(dataframe: pd.DataFrame, options: dict[str, str]) -> pd.DataFrame:
        ...
        filtered_df: pd.DataFrame = dataframe[
            ...
            (dataframe['MONTH'].isin(options['months'])) &
    ...
    def merge_dataframes(dataframe: pd.DataFrame, binnacle: pd.DataFrame) -> pd.DataFrame:
        ...
        merged_df['VALOR'] = pd.to_numeric(merged_df['VALOR'].str.replace('$', '').str.replace(',', ''))
    ...
    @staticmethod
    def add_additional_discount_column(dataframe: pd.DataFrame) -> pd.DataFrame:
        """
